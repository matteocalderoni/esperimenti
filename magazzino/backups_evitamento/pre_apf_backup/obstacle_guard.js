...
function checkAndHandleObstacles(options = {}) {
  ...
  const glideThreshold  = options.glideThreshold  || 0.70;
  const dangerThreshold = options.dangerThreshold || 0.22;
  const STUCK_LIMIT     = 40;   // frame in zona ostacolo prima di attivare fuga
  const ESCAPE_FRAMES   = 45;   // durata della manovra di fuga aggressiva
  ...
  // Calcola direzione di fuga verso il lato PIÙ LIBERO
  let steerDir;
  if (Math.abs(pan) > 5 && mode === 'automatic') {
    steerDir = (pan > 0) ? -1 : 1;
  } else if (leftD < rightD - 0.03) {
    steerDir = 1;  // Ostacolo a sinistra -> Sterza a Destra
  } else if (rightD < leftD - 0.03) {
    steerDir = -1; // Ostacolo a destra -> Sterza a Sinistra
  } else {
    steerDir = (robotState.steering >= 0) ? 1 : -1;
  }
  ...
  if (robotState.stuckEscaping) {
    robotState.stuckFrames = Math.max(0, robotState.stuckFrames - 1);
    if (robotState.stuckFrames < STUCK_LIMIT - ESCAPE_FRAMES) {
      robotState.stuckEscaping = false;
    }
    robotState.speed    = -1.2;
    robotState.steering = steerDir * 0.14;
    robotState.ledColor = (Math.floor(Date.now() / 150) % 2 === 0) ? '#ff4400' : '#000000';
    return true;
  }

  // 4. Fascia Rossa (dist < dangerThreshold) -> Disimpegno in retromarcia
  ...
  // 5. Fascia Gialla (dangerThreshold <= dist < glideThreshold) -> Decelerazione e sterzata progressive
  if (dist < glideThreshold) {
    const urgency = (glideThreshold - dist) / (glideThreshold - dangerThreshold);
    robotState.speed    = Math.max(0.4, 1.6 - (1.1 * urgency));
    robotState.steering = steerDir * (0.04 + 0.08 * urgency);
