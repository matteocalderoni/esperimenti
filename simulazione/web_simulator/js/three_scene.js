...
function initThreeFPV() {
  ...
  try {
    ...
    threeScene.add(new THREE.AmbientLight(0xffffff, 3.2));
...
function getThreeFPSnapshot() {
  if (threeInitialized && threeRenderer) {
    const curW = threeRenderer.domElement.width, curH = threeRenderer.domElement.height;
    threeRenderer.setSize(640, 480, false);
    threeRenderer.render(threeScene, threeCamera);
    const dataUrl = threeRenderer.domElement.toDataURL('image/jpeg', 0.95);
    threeRenderer.setSize(curW, curH, false);
    return dataUrl;
  }
  const fpv = document.getElementById('fpvCanvas');
  return (fpv && typeof fpv.toDataURL === 'function') ? fpv.toDataURL('image/jpeg', 0.95) : null;
}
