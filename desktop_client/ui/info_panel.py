...
class InfoPanel:
    def __init__(self, parent, x, y, width, height, connect_callback):
        ...
        self.card = tk.LabelFrame(parent, text=" Connettività & Telemetria ", bg='#121212', fg='#29B6F6', bd=1, relief='solid', font=('Helvetica', 10, 'bold'))
        self.card.place(x=x, y=y, width=width, height=height)
        
        # Connettività Input Layout (coordinati locali alla card)
        self.lbl_ip = tk.Label(self.card, text='Indirizzo IP:', fg=settings.color_text, bg='#121212')
        ...
        self.btn_connect = tk.Button(self.card, width=8, text='Connetti', fg=settings.color_btn_text, bg=settings.color_btn, relief='ridge')
        ...
        self.lbl_status = tk.Label(self.card, width=14, text='Disconnesso', fg=settings.color_text, bg='#F44336', font=('Helvetica', 9, 'bold'))
        self.lbl_status.place(x=360, y=32)
        
        self.lbl_temp = tk.Label(self.card, width=20, text='Temp CPU: -- ℃', fg=settings.color_text, bg='#212121', anchor='w', padx=8)
        self.lbl_temp.place(x=500, y=10)
        
        self.lbl_usage = tk.Label(self.card, width=20, text='Uso CPU: --', fg=settings.color_text, bg='#212121', anchor='w', padx=8)
        self.lbl_usage.place(x=500, y=40)
        
        self.lbl_ram = tk.Label(self.card, width=20, text='Uso RAM: --', fg=settings.color_text, bg='#212121', anchor='w', padx=8)
    ...
    def update_info(self, temp, usage, ram):
        ...
        self.lbl_temp.config(text=f'Temp CPU: {temp}℃')
        self.lbl_usage.config(text=f'Uso CPU: {usage}')
        self.lbl_ram.config(text=f'Uso RAM: {ram}')
